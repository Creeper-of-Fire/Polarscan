...
class Polarscan:
    ...
    def save_assets(
        ...
        with self._lock:
            ...
            current_paths = {a.path for a in polaroid.assets}
            new_paths = {a["path"] for a in new_assets}
            if not new_paths.issubset(current_paths):
                extra = new_paths - current_paths
                raise ValueError(
                    f"save_assets 不允许新增 path: {sorted(extra)}"
                )
            if not new_paths:
                raise ValueError("assets 不能为空")

            # 保留每个 path 的 hash (从当前 polaroid 取), 不重算
            hash_by_path = {a.path: a.hash for a in polaroid.assets}
            rebuilt: list[Asset] = []
            for a_dict in new_assets:
                rebuilt.append(
                    Asset(
