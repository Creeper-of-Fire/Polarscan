...
class MakePolaroidIdTest(unittest.TestCase):
    def test_ascii_basic(self) -> None:
        ...
        pid = make_polaroid_id("2025-10-18", "strawberry")
        self.assertTrue(pid.startswith("2025-10-18_strawberry_"))
        self.assertRegex(pid, _HEX_SUFFIX)

    def test_chinese_primary_char_preserved(self) -> None:
        """中文角色名应原样保留在 id 中，不被吞成 -。"""
        pid = make_polaroid_id("2026-08-04", "中文名")
        self.assertTrue(pid.startswith("2026-08-04_中文名_"), msg=f"got {pid!r}")
        self.assertRegex(pid, _HEX_SUFFIX)

    def test_chinese_only(self) -> None:
        """纯中文 char + 缺日期也要保留中文。"""
        pid = make_polaroid_id(None, "草莓")
        self.assertTrue(pid.startswith("nostamp_草莓_"), msg=f"got {pid!r}")
    ...
    def test_suffix_is_unique_hex(self) -> None:
        """6 位 hex 后缀每次都不一样。"""
        seen = {make_polaroid_id("2026-08-04", "strawberry") for _ in range(20)}
        self.assertEqual(len(seen), 20)


class ParsePrimaryCharTest(unittest.TestCase):
    def test_explicit_char_tag(self) -> None:
        self.assertEqual(parse_primary_char(["char:中文", "shot:solo"]), "中文")

    def test_fallback_when_no_prefix(self) -> None:
        self.assertEqual(parse_primary_char(["中文名", "shot:solo"]), "中文名")
